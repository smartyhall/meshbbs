...
impl BackupManager {
    ...
    pub fn apply_retention_policy(&mut self) -> io::Result<Vec<String>> {
        ...
        let mut daily: Vec<_> = self
            ...
            .collect();
        let mut weekly: Vec<_> = self
            .backups
            .values()
            .filter(|b| b.backup_type == BackupType::Weekly)
            .collect();
        let mut monthly: Vec<_> = self
            .backups
            .values()
    ...
    pub fn get_stats(&self) -> BackupStats {
        ...
        if let Some(latest) = self.backups.values().max_by_key(|b| b.created_at) {
...
mod tests {
    ...
    fn test_create_backup() {
        ...
        let metadata = manager
            .create_backup(Some("test_backup".to_string()), BackupType::Manual)
            .unwrap();
    ...
    fn test_verify_backup() {
        ...
        let mut manager =
            BackupManager::new(db_path, backup_path, RetentionPolicy::default()).unwrap();
    ...
    fn test_retention_policy() {
        ...
        for i in 0..5 {
            manager
                .create_backup(Some(format!("daily_{}", i)), BackupType::Daily)
                .unwrap();
    ...
    fn test_list_backups() {
        ...
        manager
            .create_backup(Some("backup1".to_string()), BackupType::Manual)
            .unwrap();
        manager
            .create_backup(Some("backup2".to_string()), BackupType::Daily)
            .unwrap();
        manager
            .create_backup(Some("backup3".to_string()), BackupType::Weekly)
            .unwrap();
        ...
        let backup3 = backups
            .iter()
            .find(|b| b.name == Some("backup3".to_string()));
