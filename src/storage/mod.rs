...
use crate::validation::{
    safe_filename, sanitize_message_content, secure_json_parse, secure_message_path,
    secure_topic_path, validate_file_size, validate_sysop_name, validate_topic_name,
    validate_user_name,
};
...
impl Storage {
    ...
    #[allow(dead_code)]
    pub fn set_topic_levels(&mut self, map: std::collections::HashMap<String, (u8, u8)>) {
        self.topic_levels = map;
    }
    pub fn get_topic_levels(&self, topic: &str) -> Option<(u8, u8)> {
        self.topic_levels.get(topic).copied()
    }
    #[allow(dead_code)]
    pub fn set_max_message_bytes(&mut self, max: usize) {
        self.max_message_bytes = max.min(230);
    }
    ...
    fn argon2_configured(&self) -> &Argon2<'static> {
        &self.argon2
    }
    ...
    pub async fn register_user(
        ...
        username: &str,
        password: &str,
        maybe_node: Option<&str>,
    ) -> Result<()> {
        // Validate username with security rules
        let validated_username =
            validate_user_name(username).map_err(|e| anyhow!("Invalid username: {}", e))?;
    ...
    pub async fn upsert_user_with_hash(
        &mut self,
        username: &str,
        password_hash: &str,
        level: u8,
        ...
        let validated_username =
            validate_sysop_name(username).map_err(|e| anyhow!("Invalid username: {}", e))?;
        let users_dir = Path::new(&self.data_dir).join("users");
        fs::create_dir_all(&users_dir).await?;
        let safe_name = safe_filename(&validated_username);
        let user_file = users_dir.join(format!("{}.json", safe_name));
    ...
    pub async fn bind_user_node(&mut self, username: &str, node_id: &str) -> Result<User> {
        ...
        if user.node_id.is_none() {
            user.node_id = Some(node_id.to_string());
        }
    ...
    pub async fn update_user_password(&mut self, username: &str, new_password: &str) -> Result<()> {
        if new_password.len() < 8 {
            return Err(anyhow!("Password too short (min 8)"));
        }
    ...
    pub async fn update_user_level(
        ...
        let action = if new_level > old_level {
            "PROMOTE"
        } else {
            "DEMOTE"
        };
        let details = format!("Level changed from {} to {}", old_level, new_level);
        self.log_admin_action(action, Some(username), actor, Some(&details))
            .await?;
    ...
    pub async fn store_message(
        ...
        let validated_topic =
            validate_topic_name(topic).map_err(|e| anyhow!("Invalid topic name: {}", e))?;
        ...
        if !self.topic_exists(&validated_topic) {
            return Err(anyhow!(
                "Topic '{}' does not exist. Topics must be created by a sysop.",
                validated_topic
            ));
        }
        ...
        if let Some(topic_config) = self.get_topic_config(&validated_topic) {
            let author_level = if let Some(user) = self.get_user(author).await? {
                user.user_level
            } else {
                0
            };
            if author_level < topic_config.post_level {
                return Err(anyhow!(
                    "Insufficient privileges to post to this topic (required level: {})",
        ...
        if !topic_dir.exists() {
            return Err(anyhow!(
                "Topic directory missing - topic may need to be recreated by sysop"
            ));
        }
        ...
        while let Some(area_entry) = area_entries.next_entry().await? {
            if area_entry.file_type().await?.is_dir() {
                ...
                if let Some(area_name) = area_entry.file_name().to_str() {
                    ...
                    warn!(
                        "Skipping directory with invalid name: {:?}",
                        area_entry.path()
                    );
                    continue;
                }
                    ...
                    if message_entry.path().extension().and_then(|s| s.to_str()) == Some("json") {
                        ...
                        if let Ok(metadata) = message_entry.metadata().await {
                            if metadata.len() > 1_000_000 {
                                // 1MB limit per message file
                                warn!(
                                    "Skipping oversized message file: {:?}",
                                    message_entry.path()
                                );
                                continue;
                            }
    ...
    pub async fn count_messages_since_in_topic(
        &self,
        topic: &str,
        since: DateTime<Utc>,
    ) -> Result<u32> {
        let mut count: u32 = 0;
        let topic_dir = Path::new(&self.data_dir)
            .join("messages")
            .join(safe_filename(topic));
        if !topic_dir.exists() {
            return Ok(0);
        }
    ...
    pub async fn get_deletion_audit_page(
        &self,
        page: usize,
        page_size: usize,
    ) -> Result<Vec<DeletionAuditEntry>> {
        if page == 0 {
            return Ok(vec![]);
        }
        let path = Path::new(&self.data_dir).join("deletion_audit.log");
        ...
        let mut entries: Vec<DeletionAuditEntry> = content
            .lines()
    ...
    pub async fn get_admin_audit_page(
        &self,
        page: usize,
        page_size: usize,
    ...
    pub fn lock_topic(&mut self, topic: &str) {
        self.locked_topics.insert(topic.to_string());
    }
    /// Unlock a message topic
    pub fn unlock_topic(&mut self, topic: &str) {
        self.locked_topics.remove(topic);
    }
    /// Check if topic locked
    pub fn is_topic_locked(&self, topic: &str) -> bool {
        self.locked_topics.contains(topic)
    }
    ...
    pub async fn create_topic(
        ...
        self.runtime_topics
            .topics
            .insert(topic_id.to_string(), topic_config);
    ...
    pub async fn modify_topic(
        ...
        read_level: Option<u8>,
        post_level: Option<u8>,
    ) -> Result<()> {
        ...
        if let Some(n) = name {
            topic_config.name = n.to_string();
        }
        if let Some(d) = description {
            topic_config.description = d.to_string();
        }
        if let Some(r) = read_level {
            topic_config.read_level = r;
        }
        if let Some(p) = post_level {
            topic_config.post_level = p;
        }
    ...
    pub async fn get_user(&self, username: &str) -> Result<Option<User>> {
        let safe_filename = safe_filename(username);
        let user_file = Path::new(&self.data_dir)
            .join("users")
        ...
        if messages_dir.exists() {
            ...
            while let Some(area_entry) = area_entries.next_entry().await? {
                if area_entry.file_type().await?.is_dir() {
                    let mut message_entries = fs::read_dir(area_entry.path()).await?;
                    while let Some(message_entry) = message_entries.next_entry().await? {
                        if message_entry.file_type().await?.is_file()
                            && message_entry
                                .path()
                                .extension()
    ...
    pub async fn mark_welcome_shown(
        &self,
        username: &str,
        registration_welcome: bool,
        first_login_welcome: bool,
    ) -> Result<()> {
        let user_file = std::path::Path::new(&self.data_dir)
            .join("users")
    ...
    pub fn list_subtopics(&self, parent_id: &str) -> Vec<String> {
        let mut v: Vec<String> = self
            ...
            .topics
            .iter()
            .filter_map(|(id, cfg)| match &cfg.parent {
                Some(p) if p == parent_id => Some(id.clone()),
                _ => None,
            })
            .collect();
        v.sort();
    ...
    pub async fn set_message_title(
        &self,
        topic: &str,
        id: &str,
        title: Option<&str>,
