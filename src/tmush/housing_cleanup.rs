...
use crate::tmush::{TinyMushError, TinyMushStore, WorldConfig};
...
pub async fn check_and_cleanup_housing(
    ...
    info!(
        "Housing cleanup check #{}: scanning all instances",
    ...
    for instance_id in instance_ids {
        if let Ok(mut instance) = tmush_store.get_housing_instance(&instance_id) {
            ...
            let housing_name = tmush_store
                .get_housing_template(&instance.template_id)
                .ok()
                .map(|t| t.name)
            ...
            if let Ok(Some(owner)) = storage.get_user(owner_username).await {
                ...
                if days_inactive >= config.items_to_reclaim_days && instance.active {
                    ...
                    if let Some(ref notify) = notification_fn {
                        let message = world_config
                            .msg_housing_inactive_warning
                ...
                if !instance.active && instance.inactive_since.is_some() {
                    let days_in_reclaim = now
                        .signed_duration_since(instance.inactive_since.unwrap())
                        .num_days();
    ...
    for instance_id in instance_ids {
        if let Ok(mut instance) = tmush_store.get_housing_instance(&instance_id) {
            ...
            let template = match tmush_store.get_housing_template(&instance.template_id) {
                ...
                Err(e) => {
                    warn!(
                        "Failed to get template {} for instance {}: {}",
            ...
            let mut player = match tmush_store.get_player(owner_username) {
                ...
                Err(e) => {
                    warn!(
                        "Failed to get player {} for housing {}: {}",
            ...
            if total_value >= cost {
                ...
                if let Err(e) = tmush_store.put_player(player.clone()) {
                    warn!(
                        "Failed to update player {} after housing payment: {}",
                        owner_username, e
                    );
                    payments_failed += 1;
                    continue;
                ...
                info!(
                    "Successfully processed payment for housing {} ({}): {} credits",
...
impl AbandonedHousingInfo {
    ...
    pub fn status_message(&self) -> String {
        match self.days_inactive {
            0..=29 => format!(
                "⚠️  Warning: {} days inactive (approaching 30-day threshold)",
                self.days_inactive
            ),
