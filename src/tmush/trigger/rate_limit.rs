    ...
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            RateLimitReason::ObjectLimitExceeded { object_id, limit } => {
                write!(
                    f,
                    "Object {} has exceeded the rate limit of {} executions per minute",
                    object_id, limit
                )
            }
            RateLimitReason::PlayerCooldown {
                player_name,
                seconds_remaining,
            } => {
                write!(
                    f,
                    "Player {} must wait {} seconds before triggering again",
            ...
            RateLimitReason::GlobalDisabled => {
                write!(
                    f,
                    "Trigger system is currently disabled by an administrator"
                )
            }
        }
    }
        ...
        // Check player cooldown
        {
            ...
            if entry.is_in_cooldown(now) {
                let seconds_remaining = PLAYER_COOLDOWN_SECONDS
                    - now
                        .signed_duration_since(entry.last_execution)
                        .num_seconds();
...
mod tests {
    ...
    fn test_object_rate_limit() {
        ...
        for i in 0..MAX_EXECUTIONS_PER_OBJECT_PER_MINUTE {
            assert!(limiter
                .check_allowed("obj1", &format!("player{}", i))
                .is_ok());
        ...
        assert!(matches!(
            result.unwrap_err(),
            RateLimitReason::ObjectLimitExceeded { .. }
        ));
    }
    ...
    fn test_disable_object() {
        ...
        assert!(matches!(
            result.unwrap_err(),
            RateLimitReason::TriggerDisabled { .. }
        ));
