...
use crate::tmush::trigger::{TriggerRateLimiter, RateLimitStats};
...
pub fn check_trigger_allowed(object_id: &str, player_name: &str) -> Result<(), crate::tmush::trigger::RateLimitReason> {
    get_rate_limiter().read().unwrap().check_allowed(object_id, player_name)
}
...
pub fn record_trigger_execution(object_id: &str, player_name: &str) {
    get_rate_limiter().read().unwrap().record_execution(object_id, player_name);
}
...
pub fn is_object_trigger_disabled(object_id: &str) -> bool {
    get_rate_limiter().read().unwrap().is_object_disabled(object_id)
}
...
pub fn set_trigger_system_enabled(enabled: bool) {
    get_rate_limiter().read().unwrap().set_global_enabled(enabled);
}
...
pub fn format_trigger_stats(store: &TinyMushStore) -> Vec<String> {
    ...
    output.push(format!("Global Status: {}", if stats.globally_enabled { "ENABLED" } else { "DISABLED" }));
    ...
    if stats.disabled_objects > 0 {
        ...
        for (obj_id, disabled_at) in get_disabled_objects() {
            ...
            let name = store.get_object(&obj_id)
                .ok()
                .map(|obj| obj.name.clone())
                .unwrap_or_else(|| obj_id.clone());
            output.push(format!("  - {} (disabled at {})", name, disabled_at.format("%Y-%m-%d %H:%M:%S")));
        }
    }
