...
impl NaturalLanguageCompiler {
    ...
    fn compile_block(&mut self) -> Result<AstNode, String> {
        ...
        loop {
            ...
            if is_indented
                && !matches!(self.current(), NaturalToken::Indent | NaturalToken::Newline)
            {
                break;
            }
            ...
            if !self.is_at_end()
                && !matches!(
                    self.current(),
                    NaturalToken::Otherwise | NaturalToken::Newline
                )
            {
    ...
    fn compile_say_to_room(&mut self) -> Result<AstNode, String> {
        ...
        let text = match self.current() {
            ...
            token => {
                return Err(format!(
                    "Expected text after 'Say to room', found {}",
                    token
                ))
            }
        };
    ...
    fn compile_give(&mut self) -> Result<AstNode, String> {
        ...
        match self.current() {
            ...
            token => Err(format!(
                "Expected item name or number after 'Give player', found {}",
                token
            )),
        }
    }
    ...
    fn compile_take(&mut self) -> Result<AstNode, String> {
        ...
        let item = match self.current() {
            ...
            token => {
                return Err(format!(
                    "Expected item name after 'Take from player', found {}",
                    token
                ))
            }
        };
    ...
    fn compile_teleport(&mut self) -> Result<AstNode, String> {
        ...
        let room = match self.current() {
            ...
            token => {
                return Err(format!(
                    "Expected room name after 'Teleport player to', found {}",
                    token
                ))
            }
        };
    ...
    fn compile_unlock(&mut self) -> Result<AstNode, String> {
        ...
        let direction = match self.current() {
            ...
            token => {
                return Err(format!(
                    "Expected direction after 'Unlock', found {}",
                    token
                ))
            }
        };
...
impl Parser {
    ...
    fn parse_comparison(&mut self) -> Result<AstNode, String> {
        ...
        while let Token::Equal
        | Token::NotEqual
        | Token::Greater
        | Token::Less
        | Token::GreaterEqual
        | Token::LessEqual = self.current()
        {
...
mod tests {
    ...
    fn test_nl_tokenize_multi_word_keywords() {
        ...
        assert_eq!(
            tokens[1],
            NaturalToken::Text("Everyone hears this".to_string())
        );
    }
    ...
    fn test_nl_compile_condition_player_has_quest() {
        ...
        match ast {
            AstNode::Ternary { condition, .. } => match *condition {
    ...
    fn test_detect_natural_language() {
        ...
        assert_eq!(
            detect_syntax_type("If player has quest ancient_ruins: Say \"Found\""),
            SyntaxType::Natural
        );
        assert_eq!(
            detect_syntax_type("Give player 50 health"),
            SyntaxType::Natural
        );
        assert_eq!(
            detect_syntax_type("Say to room \"Everyone hears this\""),
            SyntaxType::Natural
        );
        assert_eq!(detect_syntax_type("Unlock north"), SyntaxType::Natural);
        assert_eq!(
            detect_syntax_type("Otherwise:\n  Say \"No\""),
            SyntaxType::Natural
        );
    }
    ...
    fn test_detect_advanced_dsl() {
        ...
        assert_eq!(
            detect_syntax_type("message(\"Hello!\")"),
            SyntaxType::Advanced
        );
        assert_eq!(
            detect_syntax_type("has_item(\"key\") ? message(\"Yes\") : message(\"No\")"),
            SyntaxType::Advanced
        );
        assert_eq!(
            detect_syntax_type("heal(50) && message(\"Healed!\")"),
            SyntaxType::Advanced
        );
