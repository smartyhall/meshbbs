...
fn housing_full_lifecycle() {
    ...
    let studio = store
        .get_housing_template("studio_apartment")
        .expect("get studio template");
    ...
    let instance = store
        .clone_housing_template("studio_apartment", "alice")
    ...
    let mut instance = store
        .get_housing_instance(&instance.id)
        .expect("get instance");
    instance.guests.push("bob".to_string());
    store
        .put_housing_instance(&instance)
        .expect("save instance");

    // 5. Verify guest access
    let guest_instances = store
        .get_guest_housing_instances("bob")
        .expect("get guest houses");
    assert_eq!(guest_instances.len(), 1);
    assert_eq!(guest_instances[0].id, instance.id);

    // 6. Verify owner's housing list
    let owner_instances = store
        .get_player_housing_instances("alice")
        .expect("get owner houses");
    ...
    let locked_room = store
        .get_room(&instance.entry_room_id)
        .expect("get locked room");
    ...
    let guest_instances = store
        .get_guest_housing_instances("bob")
        .expect("get guest houses after removal");
...
fn housing_template_limits() {
    ...
    let _instance1 = store
        .clone_housing_template("studio_apartment", "alice")
        .expect("clone for alice");
    let _instance2 = store
        .clone_housing_template("studio_apartment", "bob")
        .expect("clone for bob");

    // Verify count
    let count = store
        .count_template_instances("studio_apartment")
...
fn housing_multiple_rooms() {
    ...
    let template = store
        .get_housing_template("basic_apartment")
        .expect("get template");
    assert_eq!(
        template.rooms.len(),
        3,
        "Template should have 3 rooms defined"
    );
...
fn housing_reclaim_box() {
    ...
    store
        .put_housing_instance(&instance)
        .expect("save with reclaim items");
...
fn housing_deletion_cleanup() {
    ...
    store
        .put_housing_instance(&instance)
        .expect("save with guest");

    // Verify guest index exists
    let guest_houses = store
        .get_guest_housing_instances("bob")
        .expect("get guest houses");
    ...
    // Delete housing
    store
        .delete_housing_instance(&instance.id)
        .expect("delete housing");

    // Verify guest index cleaned up
    let guest_houses = store
        .get_guest_housing_instances("bob")
        .expect("get guest houses after delete");
...
fn housing_template_seeding_idempotent() {
    ...
    // First seeding
    let count1 = store
        .seed_housing_templates_if_needed()
        .expect("first seed");
    ...
    let template_ids = store
        .list_housing_templates()
        .expect("list templates after reseed");
    assert_eq!(
        template_ids.len(),
        initial_count,
