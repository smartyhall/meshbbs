...
async fn help_broadcasts_public_commands() {
    ...
    let dms: Vec<_> = msgs
        .iter()
        .filter(|(to, _)| to == &node_id.to_string())
        .collect();
    assert_eq!(dms.len(), 1, "Expected exactly one DM, got: {:?}", dms);

    let dm_body = &dms[0].1;
    assert!(
        dm_body.contains("REGISTER") && dm_body.contains("LOGIN"),
        "DM should contain BBS instructions, got: {}",
        dm_body
    );
    ...
    if !broadcasts.is_empty() {
        ...
        assert!(
            broadcast_body.contains("Public Commands"),
            "Broadcast should show public commands, got: {}",
            broadcast_body
        );
        assert!(
            broadcast_body.contains("^SLOT"),
            "Broadcast should mention slot machine command, got: {}",
            broadcast_body
        );
        assert!(
            broadcast_body.contains("^8BALL"),
            "Broadcast should mention 8-ball command, got: {}",
            broadcast_body
        );
        assert!(
            broadcast_body.contains("^FORTUNE"),
            "Broadcast should mention fortune command, got: {}",
            broadcast_body
        );
    } else {
...
async fn help_dm_and_broadcast_content() {
    ...
    server
        .route_text_event(public_evt)
        .await
        .expect("route public help lowercase");
    ...
    let dm = &dms[0].1;
    assert!(
        dm.contains("REGISTER"),
        "DM should mention REGISTER command"
    );
    assert!(dm.contains("LOGIN"), "DM should mention LOGIN command");
    assert!(
        dm.contains("Type HELP in DM"),
        "DM should explain how to get more help"
    );
}
