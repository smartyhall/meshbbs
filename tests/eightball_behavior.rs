...
async fn eightball_broadcasts_and_not_dm() {
    ...
    server
        ...
        .await
        .expect("route public 8ball");

    // Inspect recorded outbound messages
    let msgs = server.test_messages();
    assert!(
        msgs.iter()
            .any(|(to, body)| to == "BCAST" && body.starts_with("^8BALL ⟶ ")),
        "expected ^8BALL broadcast in test_messages, got {:?}",
        msgs
    );
...
async fn eightball_respects_cooldown() {
    ...
    server
        .route_text_event(evt2)
        .await
        .expect("second 8ball (cooldown)");

    // Only one broadcast should have been queued due to per-node cooldown
    let bcasts: Vec<_> = server
        .test_messages()
        .iter()
        .filter(|(to, body)| to == "BCAST" && body.starts_with("^8BALL ⟶ "))
        .collect();
    assert_eq!(
        bcasts.len(),
        1,
        "expected only one broadcast due to cooldown, got {:?}",
