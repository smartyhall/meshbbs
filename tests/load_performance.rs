...
async fn test_concurrent_sessions_20_realistic_rate() {
    ...
    let mut storage = Storage::new(&test_dir).await.expect("Failed to create storage");
    ...
    // Assertions
    assert!(metrics.ops_per_second > 0.0, "Should have positive throughput");
    let success_rate = (metrics.successful_ops as f64 / metrics.total_operations as f64) * 100.0;
    assert!(success_rate >= 90.0, "Should have >90% success rate, got {}%", success_rate);
...
fn test_message_chunking_237_byte_limit() {
    ...
    assert_eq!(chunks.len(), 5, "1000 bytes should split into 5 chunks of 237 bytes");
...
async fn test_database_performance_under_load() {
    ...
    // Assertions
    assert!(metrics.ops_per_second > 10.0, "Should handle >10 ops/sec (Argon2 is intentionally slow)");
    assert!(metrics.duration_secs < 30.0, "Should complete in under 30 seconds");
