...
use meshbbs::tmush::{
    achievement::{award_achievement, check_trigger, get_available_achievements, get_earned_achievements},
...
fn test_achievement_earning_flow() {
    ...
    let achievement_progress = player.achievements.iter()
...
fn test_incremental_progress() {
    ...
    let awarded = check_trigger(&store, "testuser", &AchievementTrigger::RoomVisits { required: 10 }).unwrap();
    assert!(awarded.contains(&"wanderer".to_string()));
    
    // Trigger 40 more room visits (should earn explorer)
    let awarded = check_trigger(&store, "testuser", &AchievementTrigger::RoomVisits { required: 40 }).unwrap();
    assert!(awarded.contains(&"explorer".to_string()));
    
    // Trigger 50 more room visits (should earn cartographer)
    let awarded = check_trigger(&store, "testuser", &AchievementTrigger::RoomVisits { required: 50 }).unwrap();
...
fn test_hidden_achievements() {
    ...
    let available = get_available_achievements(&store, "testuser").unwrap();
    let has_legendary = available.iter().any(|(a, _)| a.id == "legendary" && a.hidden);
...
fn test_category_filtering() {
    ...
    assert!(combat.len() >= 2);
    let has_first_blood = combat.iter().any(|(a, pa)| a.id == "first_blood" && pa.as_ref().map(|p| p.earned).unwrap_or(false));
...
fn test_social_achievements() {
    ...
    let awarded = check_trigger(&store, "testuser", &AchievementTrigger::FriendCount { required: 5 }).unwrap();
    assert!(awarded.contains(&"friendly".to_string()));
    
    // Trigger message count
    check_trigger(&store, "testuser", &AchievementTrigger::MessagesSent { required: 500 }).unwrap();
    check_trigger(&store, "testuser", &AchievementTrigger::MessagesSent { required: 500 }).unwrap();
    
    let player = store.get_player("testuser").unwrap();
    let chatterbox = player.achievements.iter()
...
fn test_economic_achievements() {
    ...
    // Trigger trade count
    let awarded = check_trigger(&store, "testuser", &AchievementTrigger::TradeCount { required: 50 }).unwrap();
...
fn test_quest_achievements() {
    ...
    let awarded = check_trigger(&store, "testuser", &AchievementTrigger::QuestCompletion { required: 1 }).unwrap();
    assert!(awarded.contains(&"quest_beginner".to_string()));
    
    // Complete 24 more quests
    check_trigger(&store, "testuser", &AchievementTrigger::QuestCompletion { required: 24 }).unwrap();
...
fn test_achievement_command_output_size() {
    ...
    for achievement in get_earned_achievements(&store, "testuser").unwrap() {
        let line = format!("✓ {} - {}\n  {}\n", achievement.name, achievement.title.as_deref().unwrap_or(""), achievement.description);
        assert!(line.len() < 200, "Achievement output too large: {} bytes", line.len());
    }
}
...
fn test_multiple_simultaneous_achievements() {
    ...
    let awarded = check_trigger(&store, "testuser", &AchievementTrigger::RoomVisits { required: 100 }).unwrap();
