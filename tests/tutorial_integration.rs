...
use meshbbs::tmush::{
    PlayerRecord, TinyMushStore, TinyMushStoreBuilder, TutorialState, TutorialStep,
    NpcRecord, NpcFlag, CurrencyAmount,
    tutorial::{
        should_auto_start_tutorial, start_tutorial, advance_tutorial_step,
        can_advance_from_location, skip_tutorial, restart_tutorial,
        distribute_tutorial_rewards, format_tutorial_status,
    },
};
...
fn seed_tutorial_npcs(store: &TinyMushStore) {
    ...
    dialog.insert("default".to_string(), "Welcome to Old Towne Mesh!".to_string());
...
fn test_location_based_progression_validation() {
    ...
    assert!(!can_advance_from_location(&TutorialStep::WelcomeAtGazebo, "gazebo_landing"));
    assert!(can_advance_from_location(&TutorialStep::WelcomeAtGazebo, "town_square"));
    assert!(can_advance_from_location(&TutorialStep::WelcomeAtGazebo, "city_hall_lobby"));
    
    // NavigateToCityHall: can only advance at city hall lobby
    assert!(!can_advance_from_location(&TutorialStep::NavigateToCityHall, "gazebo_landing"));
    assert!(!can_advance_from_location(&TutorialStep::NavigateToCityHall, "town_square"));
    assert!(can_advance_from_location(&TutorialStep::NavigateToCityHall, "city_hall_lobby"));
    assert!(!can_advance_from_location(&TutorialStep::NavigateToCityHall, "mayor_office"));
    
    // MeetTheMayor: can only advance at mayor's office
    assert!(!can_advance_from_location(&TutorialStep::MeetTheMayor, "gazebo_landing"));
    assert!(!can_advance_from_location(&TutorialStep::MeetTheMayor, "town_square"));
    assert!(!can_advance_from_location(&TutorialStep::MeetTheMayor, "city_hall_lobby"));
    assert!(can_advance_from_location(&TutorialStep::MeetTheMayor, "mayor_office"));
}
