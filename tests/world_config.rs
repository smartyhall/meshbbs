...
fn test_set_config_world_name() {
    ...
    // Set world name
    store.update_world_config_field("world_name", "My Custom World", "test_admin").unwrap();
...
fn test_set_config_welcome_message() {
    ...
    store.update_world_config_field("welcome_message", custom_welcome, "test_admin").unwrap();
...
fn test_set_config_motd() {
    ...
    // Set MOTD
    store.update_world_config_field("motd", "Server maintenance tonight at 10pm!", "test_admin").unwrap();
...
fn test_set_config_world_description() {
    ...
    store.update_world_config_field("world_description", "A persistent test world", "tester").unwrap();
...
fn test_config_persists_across_reopens() {
    ...
    // First store sets config
    {
        ...
        store1.update_world_config_field("world_description", "A persistent test world", "tester").unwrap();
    }
...
fn test_config_all_fields_can_be_updated() {
    ...
    // Update all fields
    store.update_world_config_field("world_name", "Test World", "admin1").unwrap();
    store.update_world_config_field("world_description", "Test Description", "admin2").unwrap();
    store.update_world_config_field("motd", "Test MOTD", "admin3").unwrap();
    store.update_world_config_field("welcome_message", "Test Welcome", "admin4").unwrap();
