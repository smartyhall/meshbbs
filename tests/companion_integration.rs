...
fn test_mount_and_dismount_horses() {
    ...
    use meshbbs::tmush::companion::{dismount_companion, mount_companion};
...
fn test_companion_stay_and_come_control() {
    ...
    stayed
        .behaviors
        .retain(|b| !matches!(b, CompanionBehavior::AutoFollow));
    store.put_companion(stayed).unwrap();

    let after_stay = store.get_companion(&dog.id).unwrap();
    assert!(
        !after_stay.has_auto_follow(),
        "Auto-follow should be disabled"
    );
