...
async fn test_message_id_and_crc_generation() {
    ...
    storage.register_user("alice", "password", None).await.unwrap();
    storage.create_topic("test", "Test Topic", "", 1, 1, "alice").await.unwrap();
    storage.register_user("testuser", "password", None).await.unwrap();
...
async fn test_crc_different_for_different_content() {
    ...
    assert_ne!(crc1, crc2, "Different content should produce different CRCs");
}
...
async fn test_backward_compatibility_with_old_messages() {
    ...
    assert!(messages[0].message_id.is_none(), "Old message should have no message_id");
    assert!(messages[0].crc16.is_none(), "Old message should have no crc16");
}
...
async fn test_message_id_format() {
    ...
    assert!(timestamp > year_2020_timestamp, "Timestamp should be after 2020");
}
