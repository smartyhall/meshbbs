...
use meshbbs::tmush::types::{NpcRecord, NpcFlag};
...
fn test_npc_with_dialogue_tree() {
    use meshbbs::tmush::types::{DialogNode, DialogChoice};
    ...
    tree.insert("greeting".to_string(), DialogNode::new("Welcome!")
        .with_choice(DialogChoice::new("Tell me more").goto("more"))
        .with_choice(DialogChoice::new("Goodbye").exit()));
    tree.insert("more".to_string(), DialogNode::new("Here's more info...")
    ...
    // Deserialize
    let loaded: NpcRecord = bincode::deserialize(&data).expect("deserialize with dialog_tree should work");
