...
use meshbbs::tmush::{TinyMushStore, PlayerRecord, ObjectRecord, ObjectOwner, ObjectTrigger, OwnershipReason};
...
fn setup_test_environment() -> (TempDir, TinyMushStore, String, String) {
    ...
    (temp, store, "take_drop_player".to_string(), "test_room".to_string())
}
...
fn test_take_object_fires_on_take_trigger() {
    ...
    assert!(!messages.is_empty(), "OnTake trigger should produce messages");
    assert!(messages.iter().any(|m| m.contains("glows")), 
...
fn test_drop_object_fires_on_drop_trigger() {
    ...
    let mut obj = ObjectRecord {
        ...
        owner: ObjectOwner::Player { username: player_name.clone() },
        created_at: Utc::now(),
        weight: 2,
        currency_value: CurrencyAmount::default(),
        value: 50,
    ...
    // Verify trigger executed
    assert!(!messages.is_empty(), "OnDrop trigger should produce messages");
    assert!(messages.iter().any(|m| m.contains("cracks")), 
...
fn test_take_locked_object_by_other_player_blocked() {
    ...
    let obj = ObjectRecord {
        ...
        owner: ObjectOwner::Player { username: "other_player".to_string() },
        created_at: Utc::now(),
        weight: 5,
        currency_value: CurrencyAmount::default(),
        value: 100,
    ...
    if let ObjectOwner::Player { username } = &obj.owner {
        assert_ne!(username, "take_drop_player", "Object owned by different player");
    }
}
...
fn test_ownership_transfer_on_take() {
    ...
    updated_obj.owner = ObjectOwner::Player { username: player_name.clone() };
    updated_obj.ownership_history.push(meshbbs::tmush::types::OwnershipTransfer {
    ...
    if let ObjectOwner::Player { username } = &retrieved.owner {
        assert_eq!(username, &player_name, "Ownership should transfer to player");
    } else {
        panic!("Owner should be Player variant");
    }
    
    // Verify history recorded
    assert!(!retrieved.ownership_history.is_empty(), "Should have ownership history");
    assert_eq!(retrieved.ownership_history.last().unwrap().reason, OwnershipReason::PickedUp);
}
...
fn test_ownership_transfer_on_drop() {
    ...
    let retrieved = store.get_object(&obj.id).unwrap();
    assert!(matches!(retrieved.owner, ObjectOwner::World), "Owner should be World after drop");
    
    // Verify history recorded
    assert!(!retrieved.ownership_history.is_empty(), "Should have ownership history");
    assert_eq!(retrieved.ownership_history.last().unwrap().reason, OwnershipReason::Dropped);
}
...
fn test_consumable_on_take_trigger() {
    ...
    // Verify trigger executed
    assert!(!messages.is_empty(), "OnTake trigger should produce messages");
    assert!(messages.iter().any(|m| m.contains("dissolves")), 
